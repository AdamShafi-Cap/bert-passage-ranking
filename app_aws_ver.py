...
model_desc = {
            '':'''Once you make a selection, the model will process your document. This can take some time if your document is large or you select the RoBERTa model. For fastest processing, use the Brexit document''',
           'DistillBERT':'''
...
with col1:
    method = st.radio('Choose one...',['PDF Document Upload','csv File Upload','Brexit Trade Agreement'])

### PDF Document ###

if method == 'PDF Document Upload':
    file=None
    with col1:
        st.write('Upload a PDF document which will the tool will attempt to split into paragraphs. You can remove the first few pages to prevent titles and contents pages from being included. A larger document will take longer to process.')
    with col2:
        num_pages=0
...
elif method == 'csv File Upload':
    file=None
    with col1:
        st.write('Upload a csv file with 1 column and no column names. The model will treat each row as a separate paragraph, searching your query against them.')
    with col2:
        file = st.file_uploader('Upload your document.')
    if file:
        try:
